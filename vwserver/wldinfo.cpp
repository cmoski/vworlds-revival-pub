...
void DeleteWorld(IWorld* pWorld, DWORD dwCookie)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			// Original code cast dwCookie to IUnknown* and called AddRef().
			// In modern ATL, connection point cookies are indices (not pointers),
			// so that cast crashes. Just call Unadvise with the cookie directly.
