...
template<class TYPE>
inline void ConstructElements(TYPE* pElements, INT_PTR nCount)
{
...
template<class TYPE>
inline void DestructElements(TYPE* pElements, INT_PTR nCount)
{
	for (; nCount--; pElements++)
		pElements->~TYPE();
}
...
template<class TYPE, class ARG_TYPE>
typename CVWList<TYPE, ARG_TYPE>::CVWNode*
CVWList<TYPE, ARG_TYPE>::NewNode(typename typename CVWList::CVWNode* pPrev, typename typename CVWList::CVWNode* pNext)
{
	if (m_pNodeFree == NULL)
	{
		// add another block
	...
	typename CVWList::CVWNode* pNode = m_pNodeFree;
...
template<class TYPE, class ARG_TYPE>
void CVWList<TYPE, ARG_TYPE>::FreeNode(typename CVWList::CVWNode* pNode)
{
	DestructElements<TYPE>(&pNode->data, 1);
	pNode->pNext = m_pNodeFree;
	m_pNodeFree = pNode;
...
template<class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
typename CVWMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::CVWAssoc*
CVWMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::NewAssoc()
{
	if (m_pFreeList == NULL)
	{
		...
		// chain them into free list
		typename CVWMap::CVWAssoc* pAssoc = (typename CVWMap::CVWAssoc*) newBlock->data();
		// free in reverse order to make it easier to debug
	...
	typename CVWMap::CVWAssoc* pAssoc = m_pFreeList;
...
template<class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
void CVWMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::FreeAssoc(typename CVWMap::CVWAssoc* pAssoc)
{
	DestructElements<VALUE>(&pAssoc->value, 1);
	DestructElements<KEY>(&pAssoc->key, 1);
	pAssoc->pNext = m_pFreeList;
