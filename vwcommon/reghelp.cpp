...
HRESULT SetDirectoryServiceURL(BSTR bstrURL)
{
	...
	// Open the PATHS key
	if (RegCreateKeyEx(HKEY_CURRENT_USER, gStrPathKey, 0, NULL, 
...
HRESULT	GetIsSDK(VARIANT_BOOL *vBool)
{
	...
	*vBool = VARIANT_FALSE;
	...
	if (RegCreateKeyEx(HKEY_CURRENT_USER, gStrSDKKey, 0, NULL, 
...
HRESULT GetRegLocalGroupName(BSTR* pbstrLocalGroup)
{
	...
	// Open the WORLD key
	if (RegOpenKeyEx(HKEY_CURRENT_USER, szAppKey, 0, KEY_ALL_ACCESS, &keyWorlds) == ERROR_SUCCESS)
	{
		DWORD length = 256;
		DWORD type = 0; 
