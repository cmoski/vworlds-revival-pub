...
'======================================
' Name: btnDirectory_onclick
' Desc: Fires when the button is click.
'       This routine displays a directory
'       of worlds available to be logged onto.
'       If the dialog returns a valid value the
'       list is updated and the logon
'       button onclick routine is called.
'--------------------------------------
Sub btnDirectory_onclick
	Dim strWorldURL

	strWorldURL = ""
	strWorldURL = window.showModalDialog("Directory.htm", sobjTop, "dialogHeight:326px;dialogWidth:506px;status:no;help:no")
	If strWorldURL <> "" Then
		dim iIndex, el
		
		Set iIndex = lstPreviousWorlds.Options.Item(strWorldURL)
		If iIndex Is Nothing Then
			Set el = document.CreateElement("OPTION")
			el.value = strWorldURL
			el.text = strWorldURL
			lstPreviousWorlds.add el
		End If
		
		lstPreviousWorlds.value = strWorldURL
		btnLogon_onclick
	End If
End Sub
'======================================
...
'======================================
' Name: btnOptions_onclick
' Desc: Fires when the button is clicked.
'       This routine displays a dialog that
'       lets the user set client options.
'--------------------------------------
Sub btnOptions_onclick
	window.showModalDialog "Options.htm", sobjTop, "dialogHeight:334px;dialogWidth:412px;status:no;help:no"
End Sub
'======================================
