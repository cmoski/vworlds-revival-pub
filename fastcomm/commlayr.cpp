...
CMessageFramer::~CMessageFramer()
{
	...
	if(p)
	{
		BYTE *pRet = (BYTE *)InterlockedCompareExchangePointer((volatile PVOID*)&m_pFramedBuffer, (void *)NULL, (void *)p);
		if(p == pRet)
		{
			m_SystemAllocator.free(p);
		}
...
int CConnection::_InitAsListener(USHORT nPort)
{
	...
	//set type to listen.
	if((void *)ct_EMPTY != InterlockedCompareExchangePointer(
		(volatile PVOID*)&m_ConnectType, 
...
int CConnection::_InformThisOfNewAcceptor(CConnection & Acceptor)
{
	...
	//set type.
	if((void *)ct_EMPTY != InterlockedCompareExchangePointer(
		(volatile PVOID*)&Acceptor.m_ConnectType, 
...
{
	//set type.
	if((void *)ct_ACCEPTOR_SOCKET != InterlockedCompareExchangePointer(
		(volatile PVOID*)&m_ConnectType, 
...
void CConnection::Disconnect(void)
{
	...
	if(hSocket!=INVALID_SOCKET)
	{
		...
		SOCKET hRetSocket = 
			(SOCKET)InterlockedCompareExchangePointer((volatile PVOID*)&m_hSocket, (void *)INVALID_SOCKET, (void *)hSocket);
