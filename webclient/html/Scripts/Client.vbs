...
Sub objClient_OnUIEvent(objFrom, strEventName, vntArgs)
	...
	' DEBUG: trace UI events — write to title bar AND document.title for visibility
	On Error Resume Next
	Dim dbgWantCount : dbgWantCount = -1
	If Not IsEmpty(sarrWantsEventsList) Then dbgWantCount = UBound(sarrWantsEventsList) + 1
	document.title = "EVT: " & strEventName & " logged=" & sblnLoggedOn & " wants=" & dbgWantCount
	On Error GoTo 0
